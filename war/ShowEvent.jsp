...
<section class="contact-w3ls">
	<div class="container">
		<h3 class="text-center w3layouts w3 w3l w3ls agileits agileinfo wthree w3-agileits">ALL events</h3>

<c:forEach var="row" items="${result3.rows}" > 
<h3>UserName:<c:out value="${row.user_name}"/></h3>
</c:forEach> 
<table  border="2" bordercolor="#EB5425" align=center> 
		        ...
		        <c:forEach var="row" items="${result.rows}" >  
                    <tr align=center>  
                        <td><c:out value="${row.event_id}"/></td>
                        <td><c:out value="${row.event_name}"/></td>                       
                        <td><c:out value="${row.place_id}"/></td>  
                        ...
                        <td><c:out value="${row.type_id}"/></td>  
                        <td><c:out value="${row.description}"/></td> 
                        <td><c:out value="${row.charge}"/></td>  
                        <td><c:out value="${row.user_id}"/></td>
                        <td>
                        <a href="UpdateEvent.jsp?id=${row.event_id}&uid=<%=uid%>">UPDATE</a>
                        <a href="DeleteEvent.jsp?id=${row.event_id}&uid=<%=uid%>" onclick="return confirm('are you sure?')">DELETE</a>  
 						</td>
                    </tr>  
                </c:forEach>
		        </table>
		    <button class="btn btn-primary"><a href="InsertEvent.jsp?uid=<%=uid%>">ADD Event</a></button>  
            </div>
            
            <div class="container">
            <h3 class="text-center w3layouts w3 w3l w3ls agileits agileinfo wthree w3-agileits">ALL Places</h3>    
 			<table  border="2" bordercolor="#EB5425" align=center> 
		        <tr align=center>
		        	<th>Place_id</th>
		        	<th>Coordinates_y</th>		        	
		        ...
		        <c:forEach var="row" items="${result1.rows}" >  
                    <tr align=center>  
                        <td><c:out value="${row.place_id}"/></td>
                        <td><c:out value="${row.coordinates_y}"/></td>                       
                        <td><c:out value="${row.coordinates_x}"/></td>  
                        <td><c:out value="${row.postcode}"/></td>  
                        <td><c:out value="${row.address}"/></td> 
                        <td>
                        <a href="UpdatePlace.jsp?pid=${row.place_id}&uid=<%=uid%>">UPDATE</a>
                        <a href="DeletePlace.jsp?pid=${row.place_id}&uid=<%=uid%>" onclick="return confirm('are you sure?')">DELETE</a>  
 						</td>
                    </tr>  
                </c:forEach>
		        </table>
		    <button class="btn btn-primary"><a href="InsertPlace.jsp?uid=<%=uid%>">ADD Place</a></button>  
             </div>  
